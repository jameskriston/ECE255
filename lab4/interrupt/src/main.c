...
*** Use if using button 2 rather than button 1 ***

void EXTI4_15_IRQHandler(void) {
        if (EXTI->PR & EXTI_PR_PR5)
        {
                EXTI->PR |= EXTI_PR_PR5;
                EXTI5Counter = ( EXTI5Counter + 1 ) & 15;
        }
}
*/
...
void interrupt(void) {
        ...
        same as above but make it button 2 on pin 5 idk just guessing
  ...
  //      SYSCFG_EXTILineConfig(EXTI_PortSourceGPIOA, EXTI_PinSource5);
        ...
        /* Make pin 5 do the same thing above */
        /*
        ...
        */
        ...
        /* 
        *** Use if you want to use button 2 rather than button 1
         while (1) {
                if ((EXTI5Counter & 0b1000) != 0)
                        GPIO_SetBits(GPIOA, GPIO_Pin_0);
                else
                        GPIO_ResetBits(GPIOA, GPIO_Pin_0);

                if ((EXTI5Counter & 0b0100) != 0)
                        GPIO_SetBits(GPIOA, GPIO_Pin_1);
                else
                        GPIO_ResetBits(GPIOA, GPIO_Pin_1);

                if ((EXTI5Counter & 0b0010) != 0)
                        GPIO_SetBits(GPIOA, GPIO_Pin_2);
                else
                        GPIO_ResetBits(GPIOA, GPIO_Pin_2);

                if ((EXTI5Counter & 0b0001) != 0)
                        GPIO_SetBits(GPIOA, GPIO_Pin_3);
                else
                        GPIO_ResetBits(GPIOA, GPIO_Pin_3);
        }
